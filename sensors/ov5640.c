...
int get_fw_status(sensor_t *sensor)
{
	int fw_status = read_reg(sensor->slv_addr, 0x3029);
	return fw_status;

}

void set_af(sensor_t *sensor)
{
	int ret = 0;
 	ret  = write_reg(sensor->slv_addr, 0x3022, 0x03);
	if(ret != 0)
	{
		ESP_LOGI(TAG,"set af fail");
	}
}
